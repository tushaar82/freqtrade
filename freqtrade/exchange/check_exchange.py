...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    ...
    # Check if it's a custom non-CCXT exchange that's officially supported
    if exchange in SUPPORTED_EXCHANGES and not is_exchange_known_ccxt(exchange):
        logger.info(
            f'Exchange "{exchange}" is a custom exchange officially supported '
